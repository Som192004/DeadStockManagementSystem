...
app.patch("/deadstock", async (req, res) => {
  ...
  const updatedDeadStock = await updateDeadStock(lab_id, cat_id, unit_id, new_description);
...
app.patch("/history_card", async (req, res) => {
  const { lab_id, cat_id, unit_id, problem_id, remedy_taken, work_completion_date } = req.body;
  const updatedHistoryCard = await updateHistoryCard(lab_id, cat_id, unit_id, problem_id, remedy_taken, work_completion_date);
  res.status(200).send(updatedHistoryCard);
});

// To insert new problem in history card
app.post("/history_card", async (req, res) => {
  const { lab_id, cat_id, unit_id, problem_id, problem_description, report_date, remedy_taken, work_completion_date } = req.body;
  const newProblem = await insertProblem(lab_id, cat_id, unit_id, problem_id, problem_description, report_date, remedy_taken, work_completion_date);
